...
The matcher exists to disambiguate skills against vague prompts when a plugin has many of them. `vercel-plugin` ships ~19 skills, many of which compete for the same kinds of prompts. sui-pilot shipped **5, all directly invokable as slash commands** (`/move-code-quality`, `/move-code-review`, `/oz-math`, `/specify`, `/verify`) — the roster at commit `97484c5`, which is what this analysis compares. The count is still 5 today, with a different membership. The pattern works for vercel-plugin because the disambiguation matters; it doesn't here because there's nothing to disambiguate.
