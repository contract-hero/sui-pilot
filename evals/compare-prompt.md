You are scoring a 2-way A/B comparison of sui-pilot architectures against a fixed task suite. Each version was given the same prompt against the same fixture; the only difference is which sui-pilot version was active.
...
- Results root: `RESULTS_DIR_PLACEHOLDER`
  - `<version>.sha` — git SHA recorded for that arm
...
- Versions actually run: `VERSIONS_PLACEHOLDER` (comma-separated subset of `v1,v2`).
...
<p><strong>SHAs:</strong> v1=<code>...</code>, v2=<code>...</code></p>
...
<table>
  ...
  <tbody>
    ...
    <tr><th>Total input tokens</th><td>...</td><td>...</td></tr>
...
<table>
  <thead><tr><th>Task</th><th>Category</th><th>v1</th><th>v2</th><th>Notes</th></tr></thead>
  <tbody>
...
<h2>Verdict</h2>
<p>One paragraph in plain English. Does v2 deliver measurable productivity gain over v1, or just token savings? Name the specific tasks that drove the call. If v2 wins on multi-file/ambiguous/stale-training but loses on token-pressure (or vice versa), say so.</p>
