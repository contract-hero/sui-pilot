...
keywords:
  ...
  - '@mysten/sui'
questions:
  - Which Sui client should I use?
  - 'What is the difference between gRPC, GraphQL, and JSON-RPC clients?'
  - How do I create a Sui client in TypeScript?
answer: >-
  The Sui SDK offers three client types: SuiGrpcClient (recommended, fastest), SuiGraphQLClient
  (flexible queries), and SuiJsonRpcClient (legacy). All share the Core API interface for common
  operations.
goal:
  description: >-
    Reader can choose and configure the right Sui client (gRPC, GraphQL, or JSON-RPC) for their use
    case
  requires:
    - has_frontmatter:
        - title
...
<Callout type="info">
	JSON-RPC is deprecated. See the [migration guide](/sui/migrations/sui-2.0/json-rpc-migration) if
