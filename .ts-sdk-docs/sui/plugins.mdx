...
description: Extend transaction building with reusable plugins.
keywords:
  - transaction plugins
  - plugin API
  - Transaction builder
  ...
  - extensibility
questions:
  - What are Sui SDK plugins?
  - How do I add middleware to a Sui client?
  - How do I extend Sui client functionality?
answer: >-
  Plugins let you extend Sui client behavior with middleware-style interceptors for logging,
  caching, retry logic, and custom transformations on requests and responses.
goal:
  description: Reader can extend Sui client behavior with plugins
  requires:
    - has_frontmatter:
        - title
        - description
...
<Callout type="warn">
	The `Transaction` plugin API is experimental and might change rapidly as it is being developed.
</Callout>
...
```typescript
import { TransactionCommands, Transaction } from '@mysten/sui/transactions';
...
```typescript
import { TransactionCommands, Transaction, TransactionObjectInput } from '@mysten/sui/transactions';
