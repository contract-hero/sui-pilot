...
keywords:
  ...
  - TypeScript SDK
questions:
  - What are executors in the Sui SDK?
  - How do I retry failed Sui transactions?
  - How do I execute transactions in parallel on Sui?
answer: >-
  Executors manage transaction submission with automatic retry, gas management, and parallel
  execution support for high-throughput Sui applications.
goal:
  description: >-
    Reader can use executors to manage transaction submission with retry and parallel execution
  requires:
    - has_frontmatter:
        - title
        - description
...
<Callout type="warn">
	`ParallelTransactionExecutor` is experimental and might change rapidly as it is being developed.
</Callout>
...
<Callout type="warn">
	Transaction plugins and intents might resolve their own data (such as object references) that are
