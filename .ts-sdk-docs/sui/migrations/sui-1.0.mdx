...
description: Migrate to Sui TypeScript SDK 1.0 with new features and API changes.
keywords:
  - migration
  - '1.0'
  - upgrade
  ...
  - intents
questions:
  - What changed in the Migrate to 1.0 migration?
  - How do I migrate to Migrate to 1.0?
  - How does Changes to @mysten/sui work?
  - How does @mysten/sui/bcs work?
answer: Migrate to Sui TypeScript SDK 1.0 with new features and API changes.
goal:
  description: >-
    Reader can migrate to Sui TypeScript SDK 1.0 with new features and API changes
  requires:
    - has_frontmatter:
        - title
        - description
...
We have also removed the protocol config and limits options because enforcing these limits in the
...
might find that some of the data structures have been updated.
...
By default `useSignAndExecuteTransactionBlock` will now return an object with the transaction
`digest` and the BCS encoded transaction effects:
...
broadly adopted by apps.

### Helpers

2 new helpers have been added to make it easier for apps to handle wallets that have implemented
