...
<figure>
  <iframe src="/diagrams/interactive_quilt-vs-blobs_v3.html"
    style={{width:'100%',height:'700px',border:'0'}}
    title="Batch storage with Quilt: six small files stored as six separate Walrus blobs each carry the fixed 64 MB per-blob metadata charge, totalling 384 MB, while the same six files packed into one quilt pay that charge once, and you can still retrieve each file individually as a patch by its identifier."
    loading="lazy"></iframe>
  <figcaption>
