...
goal:
  description: Reader can understand the full set of Walrus blob operations available to developers
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - How do I delete?
  - How do I use sui object and blob id utilities?
  - How do I extend a blob's storage duration?
answer: Developer guide to Walrus operations for blob management.
---

Blobs can be interacted with through familiar file system operations such as uploading, reading, downloading, and deleting.

<figure>
  <iframe src="/diagrams/interactive_blob-lifecycle_v2.html"
    style={{width:'100%',height:'660px',border:'0'}}
    title="The life of a Walrus blob: a client buys a storage resource on Sui for a number of epochs, registers the blob, erasure-codes it into slivers, distributes them to the storage node committee, collects signed confirmations from a write quorum of 2f + 1 nodes, and posts the availability certificate to Sui. Walrus then keeps the blob retrievable for the purchased storage period; extend the period to keep it available, or it expires and the system reclaims the space."
    loading="lazy"></iframe>
  <figcaption>
