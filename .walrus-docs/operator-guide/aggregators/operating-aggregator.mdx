...
keywords:
  ...
  - operations
goal:
  description: Operator can run a Walrus aggregator daemon, configure it with systemd, and support large file reads
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - How do I operate an aggregator?
  - How do I start a local daemon?
  - How do I get started with operate an aggregator?
answer: >-
  Run a Walrus aggregator to expose the [HTTP
  API](/docs/http-api/storing-blobs). The aggregator does not perform any
  onchain actions and only requires specifying the address on which it listens:.
---
